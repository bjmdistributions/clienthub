    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/40 backdrop-blur-[2px]" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-xl w-[420px] overflow-hidden" onClick={e => e.stopPropagation()}>
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
          <h2 className="text-[14px] font-semibold text-gray-900">Keyboard Shortcuts</h2>
          <kbd className="text-[10px] text-gray-400 bg-gray-100 px-1.5 py-0.5 rounded font-mono">?</kbd>
        </div>
        <div className="p-2">
          {SHORTCUTS.map(s => (
            <div key={s.key} className="flex items-center justify-between px-3 py-2 rounded-lg hover:bg-gray-50">
              <span className="text-[13px] text-gray-600">{s.desc}</span>
              <kbd className="text-[11px] text-gray-500 bg-gray-100 px-2 py-0.5 rounded font-mono">{s.key}</kbd>
            </div>
          ))}
        </div>
      </div>
