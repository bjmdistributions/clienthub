    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-line">
          ...
          <button
            onClick={onClose}
            className="text-muted hover:text-ink-2 p-1.5 rounded-lg hover:bg-surface-3 transition-colors"
          >
            <X size={16} />
          </button>
        </div>
        ...
        <div className="px-5 py-5 space-y-4">
          ...
          <div className="relative">
            ...
            {suggestions.length > 0 && !selected && (
              <div className="absolute top-full left-0 right-0 bg-surface border border-line rounded-xl shadow-[0_8px_24px_rgba(0,0,0,0.08)] mt-1 z-10 max-h-44 overflow-auto">
                {suggestions.map((c) => (
                  <button
                    key={c.id}
                    className="w-full text-left px-4 py-2.5 text-[14px] text-ink hover:bg-surface-2 transition-colors first:rounded-t-xl last:rounded-b-xl"
                    onClick={() => select(c)}
                  >
                    {c.name}
                    {c.company && (
          ...
          <div>
            ...
            />
            <div className="text-[10px] text-faint mt-1">Enter to submit · Shift+Enter for new line</div>
          </div>
          ...
          <div className="flex justify-end gap-2 pt-1">
            <button
              onClick={onClose}
              className="px-4 h-9 text-[13px] text-ink-2 border border-line rounded-lg hover:bg-surface-2 transition-colors"
            >
              Cancel
            </button>
            <button
