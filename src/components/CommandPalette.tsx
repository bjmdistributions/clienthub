...
export default function CommandPalette({ onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-start justify-center pt-[15vh] bg-black/40 backdrop-blur-[2px]" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-[560px] max-h-[60vh] overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        {results && allItems.length === 0 && query.trim() && (
          <div className="px-4 py-8 text-center text-[13px] text-gray-400">No results for "{query}"</div>
        )}
      </div>
    </div>
  );
