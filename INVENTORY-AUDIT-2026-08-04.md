...
**Status (2026-08-05): the INV-1 fix is RELEASED in v0.15.128** (commit `227013b`) — see §7.
Everything else below is still diagnosis only. The INV-2 backfill (8 lots live on the store that
Jack marked sold) has **not** been done — the fix stops new ones accumulating, it does not
retroactively pull those eight off the store.
...
|---|---|---|---|
| INV-1 | **P1** | Desktop "Mark sold" on a stale storefront listing is a no-op on the lot | **Fixed — released v0.15.128** |
...
| when | who | what |
|---|---|---|
| 2026-08-05 | Claude (session 1) | Wrote the INV-1 fix: a `("listing_stale", false)` arm in `resolve_approval_request` mirroring the server's `set_lot_sold` (writes `status='sold'` + `updated_at` and records the oplog upsert). Compiles clean. Committed as `227013b` and **released in v0.15.128** on 2026-08-05 (the concurrent edit it was waiting on had landed; both changes were diffed before the commit). INV-2 backfill still outstanding: 8 lots. |
